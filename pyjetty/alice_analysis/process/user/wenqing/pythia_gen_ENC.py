        ...
        for jetR in self.jetR_list:
            ...
            for jet_level in self.jet_levels:
                ...
                for ipoint in range(2, self.npoint+1):
                    ...
                    h = ROOT.TH2D(name, name, 500, pt_bins, 60, ptRL_bins)
                    h.GetXaxis().SetTitle('p_{T,ch jet}')
                    h.GetYaxis().SetTitle('p_{T,ch jet}R_{L}') # NB: y axis scaled by jet pt and z axis scaled by ln(jet pt) (applied jet by jet)
                    setattr(self, name, h)
                    getattr(self, hist_list_name).append(h)

                    name = 'h_ENC{}PtlnPt_JetPt_{}_R{}_trk10'.format(str(ipoint), jet_level, R_label)
                    print('Initialize histogram',name)
                    ptRL_bins = logbins(1E-3,1E2,60)
                    h = ROOT.TH2D(name, name, 500, pt_bins, 60, ptRL_bins)
                    h.GetXaxis().SetTitle('p_{T,ch jet}')
    ...
    def fill_matched_jet_histograms(self, level, jet, ref_jet, R_label):
        ...
        for ipoint in range(2, self.npoint+1):
            for index in range(cb0.correlator(ipoint).rs().size()):
                    ...
                    getattr(self, 'h_matched_ENC{}PtlnPt_JetPt_{}_R{}_trk00'.format(str(ipoint), level, R_label)).Fill(jet_pt_for_selection, jet_pt_for_scaling*cb0.correlator(ipoint).rs()[index], math.log(jet_pt_for_scaling)*cb0.correlator(ipoint).weights()[index])
            for index in range(cb1.correlator(ipoint).rs().size()):
                    ...
                    getattr(self, 'h_matched_ENC{}PtlnPt_JetPt_{}_R{}_trk10'.format(str(ipoint), level, R_label)).Fill(jet_pt_for_selection, jet_pt_for_scaling*cb1.correlator(ipoint).rs()[index], math.log(jet_pt_for_scaling)*cb1.correlator(ipoint).weights()[index])
            ...
            if self.do_matching and (jetR == self.ref_jetR):
                ...
                for index_j_ch, j_ch in enumerate(jets_ch):
                    ...
                    if imatched_p > -1 and imatched_h > -1:
                        ...
                        getattr(self, hname).Fill(j_ch.perp()/ref_parton_pt, ref_parton_pt)
                        hname = 'h_matched_JetPt_h_over_p_ratio_R{}'.format(R_label)
                        getattr(self, hname).Fill(j_h.perp()/ref_parton_pt, ref_parton_pt)
